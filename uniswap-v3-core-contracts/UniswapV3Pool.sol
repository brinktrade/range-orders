...
import './libraries/TransferHelper_V3Core.sol';
...
contract UniswapV3Pool is IUniswapV3Pool, NoDelegateCall {
    ...
    function collect(
        ...
        if (amount0 > 0) {
            ...
            TransferHelper_V3Core.safeTransfer(token0, recipient, amount0);
        }
        if (amount1 > 0) {
            position.tokensOwed1 -= amount1;
            TransferHelper_V3Core.safeTransfer(token1, recipient, amount1);
        }
    ...
    function swap(
        ...
        if (zeroForOne) {
            if (amount1 < 0) TransferHelper_V3Core.safeTransfer(token1, recipient, uint256(-amount1));

            uint256 balance0Before = balance0();
            IUniswapV3SwapCallback(msg.sender).uniswapV3SwapCallback(amount0, amount1, data);
            require(balance0Before.add(uint256(amount0)) <= balance0(), 'IIA');
        } else {
            if (amount0 < 0) TransferHelper_V3Core.safeTransfer(token0, recipient, uint256(-amount0));
    ...
    function flash(
        ...
        if (amount0 > 0) TransferHelper_V3Core.safeTransfer(token0, recipient, amount0);
        if (amount1 > 0) TransferHelper_V3Core.safeTransfer(token1, recipient, amount1);
