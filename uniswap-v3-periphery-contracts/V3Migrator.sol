...
import './libraries/TransferHelper_V3Periphery.sol';
...
contract V3Migrator is IV3Migrator, PeripheryImmutableState, Multicall, SelfPermit {
    ...
    function migrate(MigrateParams calldata params) external override {
        ...
        TransferHelper_V3Periphery.safeApprove(params.token0, nonfungiblePositionManager, amount0V2ToMigrate);
        TransferHelper_V3Periphery.safeApprove(params.token1, nonfungiblePositionManager, amount1V2ToMigrate);
        ...
        if (amount0V3 < amount0V2) {
            if (amount0V3 < amount0V2ToMigrate) {
                TransferHelper_V3Periphery.safeApprove(params.token0, nonfungiblePositionManager, 0);
            }

            uint256 refund0 = amount0V2 - amount0V3;
            if (params.refundAsETH && params.token0 == WETH9) {
                IWETH9(WETH9).withdraw(refund0);
                TransferHelper_V3Periphery.safeTransferETH(msg.sender, refund0);
            } else {
                TransferHelper_V3Periphery.safeTransfer(params.token0, msg.sender, refund0);
            }
        }
        if (amount1V3 < amount1V2) {
            if (amount1V3 < amount1V2ToMigrate) {
                TransferHelper_V3Periphery.safeApprove(params.token1, nonfungiblePositionManager, 0);
            }

            uint256 refund1 = amount1V2 - amount1V3;
            if (params.refundAsETH && params.token1 == WETH9) {
                IWETH9(WETH9).withdraw(refund1);
                TransferHelper_V3Periphery.safeTransferETH(msg.sender, refund1);
            } else {
                TransferHelper_V3Periphery.safeTransfer(params.token1, msg.sender, refund1);
            }
        }
    }
}
